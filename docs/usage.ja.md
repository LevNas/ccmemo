...
- **Git-tracked モードで複数の checkout を使う場合**：linked worktree や別マシンの clone は、それぞれ同名の `context-*.md` に追記するため、コピーが分岐して `git pull` が拒否されたり rebase が衝突したりします（[#24](https://github.com/LevNas/ccmemo/issues/24)。データは失われません）。予防するには、各 checkout で `CCMEMO_CAPTURE_CHECKOUT_SUFFIX=1` を設定します（オプトイン、既定はオフ）。capture ファイル名が `context-<タイムスタンプ>-session-<id8>.md` になり、各 checkout は自分のファイルにだけ追記します。`<id8>` はホスト名と checkout パスの一方向ハッシュで、どちらの実値もファイル名や本文には現れません。すでに分岐したファイル（capture、またはハブエントリの `see:` ブロック）を回復するには、merge/rebase で衝突したファイルに対して `python3 scripts/kb_graph.py union-recover <file>` を実行します。未コミットのローカル変更が相手の場合は `--theirs origin/main` を付けます。両側が追記のみのときに限って union し、行の欠落が無いことを検証し、書き込み前にバックアップを残します（`--dry-run` あり）。frontmatter の書換や本文の修正は拒否されるので手で解決してください。union で残った `see:` の重複は `lint` が報告します。詳細は [architecture.md](architecture.md#multiple-checkouts-in-git-tracked-mode-issue-24) と [link-graph.md](link-graph.md#union-recover-file---theirs-ref)（いずれも英語）にあります。
